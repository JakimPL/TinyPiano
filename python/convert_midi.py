...
import mido
from constants import DEFAULT_BPM, SONG_PATH, TICKS_PER_QUARTER
...
def midi_to_notes(
    midi_file: str, quantize_level: int = 0, max_duration: Optional[float] = None
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert MIDI file to our custom song format"
    )
    ...
    try:
        ...
        notes, bpm, _ = midi_to_notes(args.input, args.quantize, args.duration)
        ...
        for i, (pitch, velocity, start, duration) in enumerate(notes[:5]):
            ...
            print(
                f"  Note {i}: Pitch={pitch}, Vel={velocity}, Start={start_sec:.3f}s, Dur={dur_sec:.3f}s"
            )
